...
**稍后读保存一律走 `lib/reading/collect.ts` 的 `collectReadingItem()`（P1-01）**——「规范化 URL（extractFirstUrl）→ 去重（显式 user_id 限定 + 活跃条目）→ 抓取（失败降级仅存链接）→ 固定 8 字段插入 → `reading:item-created` 事件」的唯一入口，五个收集入口（QuickAddBar / 命令面板 / QuickAdd FAB / 批量导入 / 系统分享页）都是薄壳；新增收集入口必须复用它，不得直插 `reading_items`。浏览器扩展（`extensions/chrome/`）是第六个收集入口，走服务端内核 `lib/extension/collect.ts` 的 `collectForExtension()`（同一冻结语义的服务端版：Bearer JWT 鉴权、无 appEvents，支持把页面内检测到的视频/音频链接经 `normalizeMediaLinks` 清洗后以「页面媒体」小节追加进 content），API 为 `/api/extension/auth` 与 `/api/extension/collect`（middleware 豁免 + 路由内验 token，同 /api/cron 模式）。冻结语义与已知并发限制见该文件头注释与测试。
