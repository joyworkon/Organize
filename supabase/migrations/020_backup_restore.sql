-- Atomic empty-account restore for validated Backup V2 payloads.
...
as $$
begin
  -- Only the security-definer restore RPC enables this transaction-local mode.
  if current_setting('organize.restore_mode', true) = 'on'
...
begin
  ...
  insert into public.notes (
    id, user_id, title, content, reading_item_id, is_pinned, created_at, updated_at
  )
  select
    row.id, target_user, row.title, row.content, row.reading_item_id,
    row.is_pinned, row.created_at, row.updated_at
