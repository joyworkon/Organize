-- Per-article display preference: full_width on reading_items (same semantics as notes.full_width from 025).
-- Also replaces restore_backup_v2 so the new column survives backup/restore.

alter table reading_items
  add column if not exists full_width boolean not null default false;

-- Replace restore_backup_v2: same logic as 025 plus full_width in reading_items INSERT
...
begin
  ...
  insert into public.reading_items (
    ...
    reading_progress, is_pinned, full_width, started_reading_at, completed_reading_at,
  ...
  from jsonb_to_recordset(payload_data->'task_checklists') as row(
    id uuid, task_id uuid, content text, is_completed boolean,
