-- 086: 构思画布接入垃圾箱体系（模板：060_memo_trash.sql）
-- canvas_documents 无子资源、无收藏/分享关联，permanent_delete 直接物理删除软删行。

-- ========== A. mutate_trash：加入 canvas_document ==========
...
begin
  ...
  if p_resource_type not in ('note', 'reading_item', 'task', 'lesson', 'database', 'countdown', 'database_row', 'memo', 'canvas_document') then
  ...
  if p_action = 'soft_delete' then
    if p_resource_type = 'note' then
      ...
      where user_id = target_user and id = any(p_ids) and deleted_at is null;
    elsif p_resource_type = 'canvas_document' then
      update public.canvas_documents set deleted_at = now()
      where user_id = target_user and id = any(p_ids) and deleted_at is null;
    else
  ...
  if p_action = 'restore' then
    if p_resource_type = 'note' then
      ...
      update public.canvas_documents set deleted_at = null
      where user_id = target_user and id = any(p_ids) and deleted_at is not null;
    else
  ...
  if p_resource_type = 'note' then
    ...
    delete from public.canvas_documents where user_id = target_user and id = any(p_ids) and deleted_at is not null;
  else
...
-- ========== B. list_trash：追加 canvas_document 分组 ==========
...
begin
  ...
  if p_resource_type is not null
     and p_resource_type not in ('note', 'reading_item', 'task', 'lesson', 'database', 'countdown', 'database_row', 'memo', 'canvas_document') then
    raise exception using errcode = '22023', message = 'Invalid resource type';
  end if;
  ...
  return query
    ...
    from (
      ...
      union all
      select 'canvas_document'::text, cd.id,
             coalesce(nullif(cd.title, ''), '未命名画布')::text as title, cd.deleted_at
      from public.canvas_documents cd
      where cd.user_id = target_user and cd.deleted_at is not null
        and (p_resource_type is null or p_resource_type = 'canvas_document')
    ) deleted
...
-- 维持既有 EXECUTE 分层（与 050/060 相同口径）
