-- 047 save_mutation_log 幂等键补 note 维度
-- 背景：幂等日志此前只有 (mutation_id, user_id)，不含笔记维度。前端 bug 或重放场景下
-- 同一 mutation_id 被用于另一篇笔记的保存时，RPC 会直接返回旧笔记的历史结果，
-- 新笔记内容不落库且 revision 参照被污染。前端已改为切换笔记时重置幂等键；
-- 本迁移在服务端再加一道防线：命中日志但 note_id 不一致时按新请求处理。

alter table public.save_mutation_log
