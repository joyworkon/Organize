-- Optional per-note template. Existing notes retain the original simple appearance.
-- Save through the same revision/permission/atomic task pipeline as other page settings.
alter table public.notes add column if not exists page_template text not null default 'default'
  check (page_template in ('default', 'red-blue'));
...
begin
  ...
  set content = p_content,
      ...
      page_template = case
        when p_note_snapshot ? 'page_template'
          then coalesce(p_note_snapshot->>'page_template', 'default')
        else page_template
      end,
      small_font = case
...
begin
  ...
  -- IDs are remapped by the restore client; only update the authenticated owner's rows.
