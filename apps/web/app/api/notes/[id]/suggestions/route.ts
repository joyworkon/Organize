...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  let query = ctx.supabase!.from("note_suggestions").select("*").eq("note_id", params.id).order("created_at", { ascending: false });
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await ctx.supabase!.from("note_suggestions").update({ status: body.status }).eq("id", body.suggestionId).eq("note_id", params.id).eq("status", "pending").select().single();
