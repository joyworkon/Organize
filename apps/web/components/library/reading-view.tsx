...
// 资料库「稍后读」视图（阶段 C）：原 /library 列表能力原样保留——
// 状态 tab、标签筛选、智能排序、置顶、卡片、进度、多选批量。
// 快速添加条由页级统一输入框取代（能力覆盖：单链接 / 多链接逐条收集）；
// Esc 的多选退出经 registerEscape 上报给页面统一热键（保持旧 if/else 语义）。
...
export interface ReadingViewProps {
  ...
  refreshTick: number;
  /** 页面统一注册 Esc：返回 true 表示已消费（退出多选） */
  registerEscape: (handler: (() => boolean) | null) => void;
}

export function ReadingView({ search, refreshTick, registerEscape }: ReadingViewProps) {
  ...
  }, [filter, search, selectedTagIds, refreshTick]);
  ...
  // Esc 的多选退出上报给页面统一热键（保持「先退出多选、再清空搜索」的旧 if/else 语义）
  useEffect(() => {
    registerEscape(() => {
  ...
  }, [registerEscape, showCheckbox, exitSelection]);
        ...
        (() => {
          ...
          return (
            <EmptyState
              ...
              description={hasFilter ? "试试调整筛选条件" : "在上方输入框粘贴链接，或直接拖入文件"}
