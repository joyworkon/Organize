...
> **⚠ 历史文档（2026-09-11 起停用为状态入口）**：当前任务状态的唯一真相源是 [`docs/agent-workboard.md`](agent-workboard.md)，现行计划是 [`docs/long-term-agent-plan-2026-09-11.md`](long-term-agent-plan-2026-09-11.md)。本文件的任务表已过期，**不要按它重新领取或实施**；正文保留为历史记录。
...
| P4 | P4-01 原生端架构决策 | ✅ 完成（2026-09-01：ADR 0004——macOS 真机验证远程壳模式 + 托盘常驻落地；Windows 分发与生产加载源（frontendDist 域名被占）留待部署后处理，产物在解决前不得分发） | S | ~~P2-03~~（用户决策提前，开发版不依赖） |
...
2026-09-19 扩展：稍后读新增图片/文本/录音智能物料整理，依旧通过 `collectReadingItem` 保存；物料分支以内容指纹 URN 去重、不抓取网络，并自动关联分类与主题标签。原 URL 分支冻结语义不变。详见 [reading-material-organizer.md](reading-material-organizer.md)。
...
> ✅ 2026-09-01：macOS 部分完成——ADR 0004（Tauri v2 远程壳维持既定架构），
...
### P5-02 邀请共享与编辑 ✅（2026-08-31 完成：四卡全合，详见 PROGRESS.md）
...
4. **卡 4 = 前端**（已合并）：`NoteShareDialog` 分享面板（协作空间授权 grant/revoke + 邮箱精确邀请 `find_user_by_email` → `create_workspace`/`add_workspace_member` → `grant_resource` + 公开链接三段式）、`/shared`「与我共享」列表页（非属主笔记经 064 RLS 直读，未新造列表 RPC）+ 侧边栏条件入口、`notes/[id]` 按 `resource_role()` 切保存 RPC（owner→v1，editor→065 v2，viewer 只读：编辑器 `editable=false` + 保存守卫 + 标签/迁移收口）。mock 对齐：`resource_role` 如实返回 owner（单用户世界成立），协作管理 RPC 显式报错（`P5-02-MOCK`），不假成功。
...
- **逐域属主迁移已收尾（068 notes + 069 reading_items + 070 tasks 三域全落地）**——notes 域 `transfer_note_ownership`（同转语义：引用任务+子任务随迁、标签同名复制复用、评论/建议按 056 租户列随迁、反向引用清空/删除、接收人须先持 editor 授权）、reading_items 域 `transfer_reading_item_ownership`（高亮随迁是 NOT NULL 锚点下的唯一无损语义，标签同名复制，旧属主 notes/tasks/lessons 关联解除，接收人须先持 editor 授权；分享面板已泛化为 ResourceShareDialog，文章详情页有完整授权/邀请/移交入口）、tasks 域 `transfer_task_ownership`（任务+笔记联转：task_item_refs 双锚 FK 强制的唯一语义；移动集合 = 目标任务+后代+引用笔记+笔记再引任务的递归闭包；接收人须对目标任务与每一篇涉及笔记都持 editor；任务侧 UI 入口归后续产品卡——ShareResourceType 当前仅 note | reading_item）。
