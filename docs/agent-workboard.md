...
| B03 | 精确关系索引 R10b | L | 进行中 | B01/B02 | engineering-agent | B03-1 #273；B03-2 #274；B03-3 本 PR | **B03-1 设计定稿**（`docs/note-relations-index-design.md`）；**B03-2 迁移 078**（note_links 派生表 + 提取核心 + 触发器 diff 维护 + v2 RPC；实测修出 JSON null≠键缺失判停缺陷与游标裂缝缺陷，pgTAP 34 断言钉住；触发器开销 536KB UPDATE +4.5ms）。**B03-3 实施**：迁移 079——`rebuild_note_links_batch`（keyset by notes.id 分批、与触发器同套 diff、幂等可自愈）+ `reconcile_note_links`（只读对比期望/现存边集，漂移计数+样本 20 条封顶），均仅 service_role；驱动脚本 `apps/web/scripts/backfill-note-links.mts`（backfill/--reconcile 双模式、循环守卫、reconcile>0 退出码 1 可作守门）；基准脚本 `scripts/perf/backlinks-bench.mts`（专用账号播种 1 目标+1,000 来源+100 干扰×~2KB 正文，双路径完整性自检 1000/1000，预热 1 轮+计时 5 轮）。**验证**：pgTAP 079 12 断言（权限负例/幂等/自愈/全量对账零漂移/篡改恰报 1 漂移+样本/修复归零/批截断与 p_after 续跑），全套 33 文件 894 断言 PASS；本地栈回填 665 篇 + **对账连续两轮 mismatched=0（切读门槛达成）**；基准 v1 233.7ms（230.1–248.3）vs v2 30.8ms（29.9–31.9）全量翻页中位数——**~7.6×，v1 成本随内容总量线性（每页重复 LIKE 全扫 ~2.2MB 文本），v2 走索引恒定（开销为 10 次 PostgREST 往返）**；typecheck + Vitest 150 文件/1,135 用例 + lint 零警告 | B03-4 未实施（切读）；候选决策 3 项待用户；回填生产库属运维操作，未也不应自动执行 |
