...
- Vitest 基线：A03 后 148 文件 / 1,099 用例；A04 后 148 / 1,101；A05 后 148 / 1,108；A06 后 web 148 / 1,114 + collab-server 24；B01 后 web 149 / 1,128；B02 后 web 150 / 1,135（+7 probe 单测，本地实跑全绿）；pgTAP 31 文件 / 848 断言。以各 PR CI 复核为准。
...
| B03 | 精确关系索引 R10b | L | 进行中 | B01/B02 | engineering-agent | B03-1 #273；B03-2 本 PR | **B03-1 设计定稿**（`docs/note-relations-index-design.md`）：「有效内链」判定合同（link mark + 站点相对前缀 + 段解码后 uuid 校验；修 v1 两误报/两漏报，与客户端提取一致化）；共享来源可见性 = 查询时 `resource_role`（撤权即时生效）；`note_links` 派生表 + 触发器维护（11 类写路径盘点全覆盖）；串行 4 子 PR 路线。**B03-2 实施**：迁移 078——note_links（RLS 无 policy + service_role 收口，循 067；派生数据不进备份不进 mock）、提取核心 note_links_pct_decode_ascii / note_links_extract（jsonb_path `$.**.marks[*].attrs.href` 实测验证）、notes 触发器 sync_note_links diff 维护（删多余 + on conflict do nothing 保首见时间，无变化零写入）、get_note_backlinks_v2（keyset 游标：cursor=本页末行键 + has-more 探测；**取尽时省略 next_cursor 键**）。实测修出两处设计落地缺陷：① 初版 `next_cursor: null` 判停不可行——jsonb null ≠ SQL NULL，plpgsql `->'next_cursor'` 取 JSON null 使循环永不退出（改键缺失语义，客户端 `->` 得 SQL NULL 安全判停）；② 初版以「越过本页首行」作游标 + 严格谓词跳过该行自身产生裂缝（250 收 248；改本页末行作游标无缝无重）。pgTAP 078 34 断言（表/函数权限负例、提取正负例 9 形态、diff 幂等、251 来源游标翻页无重无漏、tie-break、翻页中插入不漂移、共享来源可见+撤权即时收敛、目标读权 42501、软删/自链/移交/硬删/missing 态、v1 回归），全套 32 文件 882 断言 PASS；web typecheck + Vitest 1135 + lint 零警告（零 web 代码改动）；触发器开销实测：536KB 无链接 UPDATE +4.5ms（110.6→115.1ms），600KB/500 边提取 35ms 为独立上界（B02 保存 88–140ms 基线可接受） | B03-3/4 未实施；候选决策 3 项待用户（阅读反链 UI / 图谱切读索引 / 锚点上下文摘要）；v2 尚未接客户端读（回填后才切） |
