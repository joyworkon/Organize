...
| A05 | 协作会话刷新与撤权（先设计） | L | 候选 | A03 | — | — | 上游限制：`@hocuspocus/provider` 4.6 无 `setToken`（BLOCKED.md P5-03 评估，2026-08-31）；需核实现装版本。**A04 新增设计输入（实测）**：① 编辑器先以非协作实例挂载、协作会话建立后重建，期间的输入随旧实例销毁丢失（用户级丢字窗口，collab.spec 曾因此时序失败，测试侧已加「等播种完成」守卫，产品修复未做）；② 播种租约 deny 后客户端无任何用户可见反馈（编辑器静默空白）；③ 「恢复本地草稿」在 ydoc 同步完成前点击会把草稿插入空文档、随后房间内容合并进来 → 整篇 CRDT 翻倍（CI 慢机实测复现，A04 测试已改走「使用服务器版本」规避）——同步/播种完成前应禁用恢复与编辑类动作 | 存量连接撤权窗口未定义未测 |
