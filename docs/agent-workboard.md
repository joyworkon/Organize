...
| B03 | 精确关系索引 R10b | L | 进行中 | B01/B02 | engineering-agent | B03-1 本 PR | 设计定稿：`docs/note-relations-index-design.md`——「有效内链」判定合同（link mark + 站点相对前缀 + 段解码后 uuid 校验；修 v1 两误报（纯文本/外站同路径）与两漏报（锚点查询串/百分号编码），并与客户端提取一致化消除服务端↔图谱分歧）；共享来源可见性 = 查询时 `resource_role('note',…)`（063 复用，撤权即时生效不泄露标题/计数）+ 目标读权门槛；`note_links` 派生表（RLS 无 policy + service_role 收口，循 067 先例；不进备份不进 mock）+ notes 触发器 diff 维护（11 类写路径盘点全覆盖：保存 RPC/PATCH/创建/恢复版本/备份恢复/软删/硬删/移交/导入/离线/移动块）；v2 RPC 稳定 keyset 游标只回元数据；回填/对账/基准方案齐备；串行 4 子 PR（设计→迁移触发器→回填对账→切读回退守门）。提取核心 SQL 已本地栈实测：正/负例全过 + 598KB/20 万字文档 ~35ms/次（B02 保存 88–140ms 基线上可接受） | B03-2/3/4 未实施；候选决策 3 项待用户（阅读反链 UI / 图谱切读索引 / 锚点上下文摘要），均不阻塞实施 |
