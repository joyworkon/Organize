...
| A05 | 协作会话刷新与撤权（先设计） | L | 进行中 | A03 | engineering-agent | A05-1 #265；A05-2（本 PR） | **上游核实（2026-09-12，推翻 BLOCKED P5-03 旧结论）**：`@hocuspocus/provider@4.6.0` 的 `token` 配置接受函数，每次 WS onOpen（含重连）与服务端 TokenSync 请求时重新求值；server 有 `connection.requestToken()` + `onTokenSync` hook 可对存量连接重验权限（抛错即 close），`connection.readOnly` 每消息检查（editor→viewer 降级无需重连）。设计定稿：`docs/collab-session-refresh-design.md`（缺陷 8 项/状态机/撤权窗口 ≤5min+17s）。**A05-2 已交付（客户端）**：token 函数（每次重连/重验现取会话，匿名恒 share:）；authenticationFailed 3 次退避重握手（disconnect+connect，不重建 provider）耗尽降级 error；GATE_TIMEOUT=10s 门控（**计时从会话建立起点算**——E2E 实测教训：原从 provider 创建起算，`auth.getUser()` 本地偶发 15s+ 挂起会把门控连同 provider 一起挂死成永久只读；且 hook 已删冗余 getUser，改用 session.user.id 取色零网络）；SIGNED_OUT/同页切号销毁重建（authEpoch）；status 状态机按 WS 实况（error 单向不翻回）；页面门控 collabResolved（编辑 + 草稿恢复对话框，对话框加协作覆盖提示）；编辑器播种锁（房间空且 DB 有内容时锁编辑到 grant/deny/内容到达，封 A04 实测「同步前输入致 markSeeded→DB 内容进不了房间→碎片反写 notes.content」数据丢失路径）；seed-deny 12s 无内容 toast；分享编辑器 error 降级提示。**验证**：Vitest 148 文件/1,107 用例（hook 9 条：token 函数/重试/门控/退出/切号）；本地完整栈协作 E2E 10/10 连续两轮（含修 A04 遗留测试时序：对话框改等现身、typeInBlock 先等可编辑）；typecheck/lint 零警告/build 双环境绿 | 服务端周期重验（A05-3）与撤权 E2E（A05-4）未做；真实生产环境未验证 |
