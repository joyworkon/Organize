...
| A05 | 协作会话刷新与撤权（先设计） | L | 进行中 | A03 | engineering-agent | 设计 PR（A05-1，进行中） | **上游核实（2026-09-12，推翻 BLOCKED P5-03 旧结论）**：`@hocuspocus/provider@4.6.0` 的 `token` 配置接受函数，每次 WS onOpen（含重连）与服务端 TokenSync 请求时重新求值；server 有 `connection.requestToken()` + `onTokenSync` hook 可对存量连接重验权限（抛错即 close），`connection.readOnly` 每消息检查（editor→viewer 降级无需重连）。**现状缺陷 8 项**（含 A04 三个实测输入：丢字窗口/播种 deny 无反馈/同步前恢复草稿 CRDT 翻倍，另发现：鉴权失败静默卡死（未监听 authenticationFailed）、长会话 lastWriterToken 1h 过期致 blob 持久化静默失败、status 状态机失真、无 SIGNED_OUT 销毁）。设计定稿：`docs/collab-session-refresh-design.md`——客户端 token 函数 + 失败重试 3 次降级 + 编辑门控（collabResolved/GATE_TIMEOUT=10s）；服务端周期重验（REAUTH_INTERVAL 默认 5min，撤权生效窗口 ≤5min+17s）；拆 A05-2 客户端 / A05-3 服务端 / A05-4 E2E 串行 PR | 全部实现与 E2E 未做（本行只记录设计完成） |
