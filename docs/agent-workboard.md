...
| B04 | 编辑器块交互拆分 | M | 候选 | A03 | — | — | R09 已拆装配与上传（block-commands/utils/extensions/use-editor-upload/editor-dialogs/table-controls 等接缝在）；剩余：主文件 2,307 行内的块选择/拖拽手柄（selectionUpdate 驱动）/菜单交互拆分。**2026-09-13 侦察**：拆分面已定位（keydown 处理器 :1166、菜单定位 :1029-1076、拖拽手柄 :1757-1765、selection 消费 :294/:372/:1680），验收需拖拽/表格/嵌套菜单/中文组合输入/Undo/上传中切页全量行为等价——当轮会话余量不足，为不开半个重构顺延下一会话整卡执行 | 顺延执行 |
