...
| 方法 | 路径 | 用途 |
| --- | --- | --- |
| POST/GET/DELETE | `/api/share` | 创建分享（note/reading_item）/ 我的分享列表 / 撤销 |
| GET | `/api/share/[token]` | 公开访问（免登录）：按 token 取资源，校验 is_public/expires_at |
...
| POST/DELETE | `/api/push/subscriptions` | Web Push 订阅登记/注销（web_push_subscriptions） |
