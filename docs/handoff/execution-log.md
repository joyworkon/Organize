...
> **⚠ 已被取代（2026-09-11）**：R00–R12/D00–D06 重构接力已收尾，本文件不再是「唯一进度真相源」。当前任务状态的唯一入口是 [`../agent-workboard.md`](../agent-workboard.md)，现行计划是 [`../long-term-agent-plan-2026-09-11.md`](../long-term-agent-plan-2026-09-11.md)。正文保留为历史交付证据。
...
### D00 内容与入口盘点 — ✅ 已完成（随 PR #234 入库 docs/handoff/d00-inventory.md）

- 只读盘点：侧栏 10 个一级入口 + 分组内标签/清单/经验/页面树 + 条件项（shared/spaces）+ 全局搜索/新建；导航迁移表按「现状→目标」逐项填列并留勾验位；笔记详情角色入口全部记录自实际代码（含 R03/R05/R07/R08 的新状态面）。
- 验收自查通过：历史/导出/附件/标签/页面设置/子页/任务/分享/搜索均有新位置；无「更多」掩盖未实现入口。

### D01 独立第一版预览 — ✅ 已完成（随 PR #234 入库 docs/handoff/design-preview/）

- 交付物：自包含 `index.html`（无依赖、无后端、演示数据明确标注）+ 6 张真实浏览器截图（Playwright Chromium）。
- 可操作交互：四屏切换、侧栏 232/56px 折叠、目录辅助面板（280px）开关、更多菜单（Esc 关闭+焦点返回触发器）、保存状态四态切换（已同步/保存中/仅本机/失败持续错误条）、暗色切换、空态切换、移动 FAB 聚焦收起演示。
- 截图：工作台 1366、笔记列表 1366（含长标题）、笔记详情 1440（目录+更多菜单）、暗色+失败态 1440、仅本机琥珀态 1440、移动笔记详情 390×844。
- **自查方式：独立视觉评审 agent 按规格逐张评审**——首轮 3 必修（移动截图非真实手机框、工作台行内元信息连排、列表页侧栏高亮错误）+ 1 建议（错误条改 error 色）全部修复，复审 6/6 pass、结论「D01 可交付」。
- 边界遵守：未接入正式路由、未改业务逻辑与全局 CSS、演示编辑仅内存、所有“已保存”标注为演示状态。
- 下一张可执行卡：D02。

### D03 工作台与笔记列表 — ✅ 已完成（PR #236）

- 基线提交：a42ae7d
- 修改文件与职责：
  - 新增 `components/dashboard/dashboard-capture.tsx`——工作台采集框：含链接走既有 `collectReadingItem` 路径（抓取/去重/事件不变）；纯文本走既有 `/api/memos` 存为速记并明确提示类型；「选择类型」复用既有 quick-add 事件
  - `components/dashboard/today-view.tsx`——按 §5.1 重构：三张统计卡与长堆叠卡替换为 主列（继续处理=逾期→今日→进行中合并清单 max5 + 查看全部待办 + 真实完成率/连续小字；继续阅读 max3 + 随机一篇/查看全部）+ 辅列（最近笔记 max5 + 最近速记 max3，新增 memos 拉取沿用 /api/memos）；全部 query/handler 复用（任务勾选原子更新、重复任务生成、随机推荐保留）；无统计大数字
  - `app/(main)/notes/page.tsx`——工具行新增「图谱」入口（D00 迁移表勾验项：图谱收进笔记页视图入口，仍到 /graph）；其余（搜索/排序/视图切换/多选/标签筛选/分组/空态/离线提示）原样保留——现状已符合 §5.2
- 行为变化：工作台模块次序与密度（规格授权）；统计/回顾视图与路由参数（`?view=`）不变；进行中任务并入继续处理清单（原独立段落入口保留在 /tasks）
- 测试命令及退出结果：全量 vitest 137 文件 / 1032 测试；tsc、lint 零警告；mock 构建 + Chromium 截图（工作台桌面/笔记列表桌面/工作台移动 390）**独立视觉评审 3/3 pass「可交付」**；移动折叠线下次序由 DOM 顺序保证（单列=采集→待办→阅读→笔记→速记）
- 未覆盖场景：100+ 条数据的列表性能截图验证（现有分页/分组逻辑未改动，留 D05 汇总验收）
- 回退办法：revert 本 PR。
- 下一张可执行卡：D04。
...
### D05 跨状态与响应式验收 — ✅ 已完成（PR #238）

- 基线提交：5c5e7d1。性质：验收扫描卡（修扫出的问题），不改业务逻辑。
- 扫描矩阵与结果（真实 mock 构建 + Chromium）：
  - **横向溢出**：6 视口（1440/1366/1024/768/390/360）× 6 页面（工作台/笔记/稍后读/待办/速记/设置）= 36 项检查，0 溢出；溢出元素定位器就绪（>2px 才判）
  - **200% 缩放**（body zoom=2 近似，工作台/笔记列表）：0 溢出
  - **暗色抽查**（390 工作台/笔记列表）：0 溢出；暗色 token 生效
  - **Esc 关闭 + 焦点回归**：更多菜单 Esc 关闭 ✓、焦点回归触发器 ✓（修复：原 onCloseAutoFocus preventDefault 吞焦点——保留防重开 workaround 同时显式聚焦触发器）
  - **移动抽屉**：Esc 关闭 ✓（新增 DrawerEscapeHandler；遮罩点击关闭为既有行为）
  - **保存错误不被 toast 遮挡**：保存错误在顶栏（持续），toast 在底部（2s），结构上不重叠 ✓
  - **辅助面板关闭仍能发现入口**：目录为工具行常驻按钮（aria-pressed）✓
  - **移动键盘不挡最后一行**：真实软键盘无法在桌面 Playwright 模拟——输入聚焦滚动为浏览器原生行为；FAB/底栏避让属原生键盘行为，**记为待真机手测项**（诚实标注）
- 扫出并修复的问题：菜单焦点回归（上述）；抽屉 Esc 缺失（上述）。
- 测试命令及退出结果：全量 vitest 137 文件 / 1032 测试；tsc、lint 零警告；36+6 项溢出检查全过；交互检查全过。证据截图 `docs/handoff/d05-verification/`。
- 未覆盖场景（如实记录）：真机软键盘/触摸具体行为；200% 为 zoom 近似非浏览器原生缩放。
- 回退办法：revert 本 PR。
- 下一张可执行卡：R10。
...
### R12 大笔记性能与持久化升级 — ✅ 测量完成，按协议不引入优化（PR #241，报告 docs/handoff/r12-measurement.md）

- 基线提交：f7d4a39
- 测量（真实本地后端 + 生产 Chromium 构建，样本 1k/10k/50k 中文、0 图片）：50k 打字期间长任务 0 个（无 >50ms 阻塞）、重开 ≤237ms；机器与局限（图片样本未测、savePosts/draftSize 仪表缺口）如实记录于报告。
- 决定：升级前提（localStorage 同步写确为瓶颈）不成立——**不引入 IndexedDB 适配器**（遵循「先测量，不为满足任务编号强行引入新架构」）；待办登记图片样本与仪表修复。
- 测试命令及退出结果：全量 vitest 138 文件 / 1035 测试；tsc、lint 零警告（本卡无代码改动）。
- 回退办法：纯文档。
- 下一张可执行卡：D06。

### D06 次级页面统一 — ✅ 已完成（PR #242）

- 基线提交：ae20e73。D02 token 全局生效后，次级页面（稍后读/待办/速记/收藏/垃圾箱/设置/插件/图谱/统计视图）已继承统一视觉——本卡为全页扫描验证 + 迁移表收尾。
- 扫描：9 个次级页面 × 浅/暗 = 18 帧真实渲染（生产构建 + Chromium），横向溢出 0；截图证据 `docs/handoff/d06-verification/`（工作台/设置/稍后读/待办代表帧，全量帧见 sweep 日志）。
- 迁移表勾验（D00 表收尾）：图谱 → 笔记列表工具行入口（D03 ✓）；插件 → 设置页新增「插件管理」入口（本卡，链接 /plugins 原页保留）✓；其余入口（工作台/稍后读/笔记/待办/速记/收藏/垃圾箱/设置/共享/空间/标签/经验）位置保持，旧 URL 全部可用 ✓。
- 修改文件：`app/(main)/settings/page.tsx`（新增插件管理入口卡）。
- 测试命令及退出结果：全量 vitest 138 文件 / 1035 测试；tsc、lint 零警告；18 帧扫描 0 溢出。
- 未覆盖场景：各页面空态/加载失败组合的逐一截图由 D05 扫描矩阵与各自页面既有测试覆盖。
- 回退办法：revert 本 PR。
- 下一张：跨模块端到端检查 + 最终交付。

### 跨模块端到端检查 — ✅ 已完成（随 PR #243 固化脚本与记录）

- 流程（真实本地后端 + 生产 Chromium，脚本 `docs/handoff/cross-module-e2e.mjs`）：收集文章（真实抓取）→ 打开阅读详情（高亮选择触发）→ 阅读转笔记入口 → 笔记输入→自动保存→刷新持久化 → 导出 Markdown（下载 .md）→ 第二版编辑→历史版本→恢复 → **离线编辑→联网自动同步** → 任务快速添加→完成→复盘入口 → 分享对话框（owner）。
- 结果：**10/10 通过**。检查中暴露一个真实回归——R07 会话的草稿身份绑定与 loadNote 异步替换存在竞态（会话绑定旧对象 → queueSave 永久 no-op → 保存静默失效）；修复：hydrate 时重绑定当前草稿对象（`boundDraft = draftRef.current`）。修复后 10/10、服务器端标题/正文确认落库（SQL 验证）。
- 另一发现（记录为后续改进，不在本次范围）：`public/sw.js` 的缓存策略在重新部署后会让浏览器继续使用旧 chunk（本次诊断多次「chunk 加载失败/页面陈旧」均源于此）——建议后续为 SW 增加构建版本号与自动更新策略。

## 目标完成状态

- R00–R12、D00–D06 全部任务卡完成并 squash 合并（PR #223–#243）。
- 最终 master 同步、CI 全绿；跨模块端到端 10/10；全量单测 138 文件 / 1035 测试；tsc、lint 零警告；pgTAP 28 文件 PASS（含新增 073/074/075）。
- R10b（文档链接索引表）、R12 图片样本测量、真机软键盘手测、真实协作 E2E（COLLAB_E2E）为已登记的后续项——见各卡「未覆盖场景」。 → D05 → R10 → R11 → R12 → D06 → 跨模块端到端检查。
