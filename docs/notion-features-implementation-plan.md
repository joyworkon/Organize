...
**M1 验收**：⌘/ 菜单出现全部新块；tsc + vitest 通过；每种新块各补一个最小单测（schema 序列化/命令，参照 `extensions/columns.test.ts`）。
...
| 里程碑 | 内容 | 依赖 | 建议 PR 粒度 |
| --- | --- | --- | --- |
| M1 | 目录/折叠标题补完/路径栏/按钮/选项卡/Mermaid/通用嵌入 | 无新表 | 每块 1 PR |
