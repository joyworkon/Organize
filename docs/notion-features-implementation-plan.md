...
- 图表（垂直/水平条形图、折线图、环状图、数据图表）✅ (#38)：`components/database/chart-view.tsx`，聚合求值器（groupBy + count/sum/avg，纯函数 + 单测）；渲染先用 SVG 自绘（量级小、无依赖），确实不够再评估引库（recharts 等需先确认）。已实现 bar_v/bar_h/line/donut 四种 SVG 自绘图 + `view-shared/aggregation.ts` 聚合求值器（含 9 例单测），零新依赖。
- 地图视图（**暂缓**）：地点属性（`{lat, lng, label}`）+ 瓦片地图（Leaflet + OSM，注意评估国内可用性与依赖体积，放最后一档）。暂缓理由：对「稍后读 + 笔记」场景非刚需（数据多为网页/笔记，少有地理属性）；且需引入项目首个重依赖 Leaflet + 解决国内瓦片可用性，成本/收益不划算。如后续确有地点类数据需求再单独立项。
- 动态视图✅ (#42)：按 `updated_at` 倒序的行动态流（数据现成，纯 UI）。已实现：行流卡片（标题 + 相对时间 + 副属性摘要），`lib/date-utils.ts` 的 formatTimeAgo 相对时间格式化器（零依赖手写），客户端按 updated_at 倒序重排（时间相同按 created_at 倒序兜底）。
- 管理面板视图✅ (#40)：多个「指标卡 + 图表块」的自由布局面板——建议在图表求值器完成后做，复用其聚合结果。已实现：指标卡（aggregate 求和）+ 图表块（复用 chart-svg.tsx）两类块，手写 HTML5 拖拽排序，每块内联编辑表单，复用 onUpdateViewConfig 持久化。
- AI 速记（**阻塞**）：依赖音频采集与转写服务。先走插件体系（`@organize/plugin-sdk` 的 `ai-action`），做成可选插件而非内核功能；需要明确转写 API 来源（当前仓库无 AI 服务密钥约定，动工前需与维护者确认）。阻塞理由：无转写 API 凭据/来源，无法落地真实功能；待确定转写服务（OpenAI Whisper / 阿里云 / 本地 whisper.cpp 等）后再单独立项。
...
| M4 ✅ | 看板/列表/画廊/日历/时间轴 + 链接的视图 | M3 | 合为 1 PR（feat/m4-database-views） |
| M5（主体完成） | 图表族✅(#38) / 管理面板✅(#40) / 动态✅(#42) / 地图（暂缓） / AI 速记（阻塞） | M4 | 每功能 1 PR；地图暂缓、AI速记阻塞，见 §6 |
