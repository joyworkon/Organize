...
| 页面标题区 | `components/layout/page-header.tsx`（图标 36px + h1 `text-xl sm:text-2xl` + 右侧 actions） | 全站页面标题的唯一实现，禁止再自建 h1 版式；**第十步起功能页页头只放「名称 + 右侧动作」，不放功能说明与计数**（组件仍保留 `description` prop 但目前无页面使用，新增页头文案前先问「这句说明是否值得占一行」）；工作台是特例（问候语 h1 + 吸顶条内的面包屑级「工作台」，样式在 globals.css `.dashboard-view-switcher`）；待办清单头是工作区面板头，保留自有样式；收藏夹页头不放面包屑（与 h1 重复，第十步已删） |
...
- **图标已收口（2026-09-17）**：源码零 `lucide-react` 引用，131 个文件统一从 `@/components/icons` 取图标；`lucide-react` 依赖项已于 2026-09-18 摘除，不再出现在 `package.json`。
