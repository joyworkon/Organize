...
## 执行记录（fix/macos-notch-runtime）

本分支已按上述结构落地第一轮运行时修复：

- Rust 壳改为在 AppKit 主线程读取 `NSScreen.screens`、`frame`、`visibleFrame`、`safeAreaInsets` 和顶部辅助区域；所有几何计算统一使用全局 points，按 display ID 重建临时窗口，显示器变化、睡眠和工作区变化会重新快照。对 macOS 12 以下系统保留运行时 API 守卫并降级为普通入口，不抬高既有最低系统版本。
- 刘海只作为定位锚点，不再绘制或覆盖摄像头区域；无刘海屏幕默认不创建顶部热区，可在面板设置中显式开启普通把手。
- Preview、Editing、Saving/Busy 和 Hidden 使用 session token 隔离；编辑中不因鼠标离开自动关闭，失焦关闭带宽限，旧 blur/save 回调不能关闭新会话。
- 主窗口关闭继续驻留，Dock 重开始终恢复主窗口；快捷键逐项注册并允许失败降级；托盘新增“快速记录”；导航使用 ready/ack 处理冷启动和慢加载。
- 速记面板增加账号代际、草稿 revision、显式用户校验、跨 WebView 数据桥和独立离线回放；4xx 拒绝会保留可恢复内容，草稿持久化失败不会再显示为已保存。
- 面板把手、减少动态效果/透明度、移动端安全区基础样式和设置项同步更新。

自动检查结果：前端 typecheck 通过；Vitest 146 个文件、1078 个测试通过；Rust `cargo check`、`cargo test`（11 个测试）和 rustfmt 通过；ESLint 无警告。真实 MacBook 机型、Intel/Apple Silicon 双架构、全屏/Stage Manager/Spaces、签名公证和更新链仍需按本报告验收矩阵实测，不能由本地编译替代。
